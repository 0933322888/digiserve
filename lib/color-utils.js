...
/**
 * Generate runtime variants from the persisted primary/accent palette.
 */
export function generateThemeColorVariants(primary, accent) {
    const primaryDarkBg = generateDarkModeVariant(primary, 15)
    const accentDarkBg = isLightColor(accent)
        ? '#2C2C2C' // If secondary is light, use dark gray for dark mode
        : generateDarkModeVariant(accent, 20)
    ...
    return {
        light: {
            ...
            accent: {
                bg: accent,
                text: getContrastingTextColor(accent)
            }
        },
        dark: {
            primary: {
            ...
            accent: {
                bg: accentDarkBg,
                text: getContrastingTextColor(accentDarkBg)
            }
        }
    }
}
