...
const OrderSchema = new mongoose.Schema(
  {
    ...
    tableId: String, // Added for dine-in
    seatNumber: String, // Added for dine-in
    orderType: { type: String, enum: ['pickup', 'delivery', 'dineIn'], required: true },
    items: [
      {
        id: String,
        name: String,
...
// Tenant Add-on Schema - for modular subscriptions
const TenantAddonSchema = new mongoose.Schema(
  {
    id: { type: String, required: true, unique: true },
    barId: { type: String, required: true, index: true },
    addonKey: { type: String, required: true }, // e.g., 'ordering', 'reservations'
    status: {
      type: String,
...
TenantAddonSchema.index({ barId: 1, addonKey: 1 }, { unique: true })

export function getTenantAddonModel() {
  return mongoose.models.TenantAddon || mongoose.model('TenantAddon', TenantAddonSchema)
}

// Restaurant (Tenant) Schema
const RestaurantSchema = new mongoose.Schema(
  {
    id: { type: String, sparse: true, unique: true }, // Optional external ID / UUID
    ...
    domain: { type: String }, // Primary custom domain (deprecated - use customDomains)
    customDomains: [{ type: String }], // Array of custom domains (e.g., ['www.restaurant.com', 'restaurant.com'])
    ...
    theme: {
      ...
      // New comprehensive color structure
      colors: {
        light: {
          primary: {
            bg: { type: String, default: '#8B0000' },
            text: { type: String, default: '#FFFFFF' }
          },
          secondary: {
            bg: { type: String, default: '#F5F5DC' },
            text: { type: String, default: '#2C3E50' }
          }
        },
        dark: {
          primary: {
            bg: { type: String, default: '#A00000' },
            text: { type: String, default: '#FFFFFF' }
          },
          secondary: {
            bg: { type: String, default: '#2C2C2C' },
      ...
      overrides: { type: Map, of: String }, // Key-value for specific overrides
    },
...
const UserSchema = new mongoose.Schema(
  {
    ...
    name: { type: String, required: true },
    role: { type: String, enum: ['super-admin', 'admin', 'manager', 'staff'], default: 'manager' },
...
const FloorPlanSchema = new mongoose.Schema(
  {
    ...
    objects: [
      {
        id: String,
        type: { type: String, enum: ['table', 'wall', 'stool', 'text', 'landmark'], required: true },
        x: { type: Number, default: 0 },
        y: { type: Number, default: 0 },
        width: { type: Number, default: 60 },
        height: { type: Number, default: 60 },
        rotation: { type: Number, default: 0 },
        label: String, // Table Number or Text
        capacity: { type: Number, default: 1 },
        shape: { type: String, enum: ['rect', 'circle', 'custom'], default: 'rect' },
...
FloorPlanSchema.index({ barId: 1, order: 1 })

export function getFloorPlanModel() {
  return mongoose.models.FloorPlan || mongoose.model('FloorPlan', FloorPlanSchema)
}
