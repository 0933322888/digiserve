 ...
 * Get all menus data
 */
export async function getAllMenus(barId) {
  console.log(`[MenuService] getAllMenus called for barId: ${barId}`)
  ...
  console.log(`[MenuService] getAllMenus found ${menus.length} menus`)
  return { menus }
}
...
export async function getActiveMenu(barId) {
  console.log(`[MenuService] getActiveMenu called for barId: ${barId}`)
  if (!barId) return null
  const menus = await db.collection('menus').find({ active: true, barId })
  const activeMenu = menus.length > 0 ? menus[0] : null
  console.log(`[MenuService] getActiveMenu result:`, activeMenu ? activeMenu.id : 'null')
  return activeMenu
}
...
export async function getMenu(barId, menuId) {
  console.log(`[MenuService] getMenu called for barId: ${barId}, menuId: ${menuId}`)
  if (!barId) return null
  const menu = await db.collection('menus').findOne({ id: menuId, barId })
  console.log(`[MenuService] getMenu result:`, menu ? 'Found' : 'Not Found')
  return menu
}
...
export async function createMenu(barId, menuData) {
  console.log(`[MenuService] createMenu called for barId: ${barId}`)
  ...
  console.log(`[MenuService] createMenu created: ${newMenu.id}`)
  return newMenu
}
...
export async function updateMenu(barId, menuId, updates) {
  console.log(`[MenuService] updateMenu called for ${menuId}`, updates)
...
export async function activateMenu(barId, menuId) {
  console.log(`[MenuService] activateMenu called for ${menuId}`)
  const menu = await getMenu(barId, menuId)
  if (!menu) {
    console.error(`[MenuService] activateMenu: Menu not found ${menuId}`)
    throw new Error('Menu not found')
  }
...
export async function cloneMenu(barId, menuId, newName) {
  console.log(`[MenuService] cloneMenu called for ${menuId} to ${newName}`)
  ...
  await db.collection('menus').insertOne(newMenu)
  console.log(`[MenuService] cloneMenu created: ${newMenu.id}`)
  return newMenu
}
...
export async function deleteMenu(barId, menuId) {
  ...
  console.log(`[MenuService] deleteMenu deleted: ${menuId}`)
  return true
}
...
export async function createSection(barId, menuId, sectionData) {
  const menu = await getMenu(barId, menuId)
  if (!menu) {
    console.error(`[MenuService] createSection: Menu not found ${menuId}`)
    throw new Error('Menu not found')
  }
...
export async function updateMenuItem(barId, menuId, sectionId, itemId, updates) {
  const menu = await getMenu(barId, menuId)
  if (!menu) {
    console.error(`[MenuService] updateMenuItem: Menu not found ${menuId}`)
    throw new Error('Menu not found')
  }
