...
import { db } from './db'
import { getBusinessHours, getSetting } from './app-settings-service'
...
export async function getSettings(barId) {
  if (!barId) {
    return {
  ...
  const maxSeats = await getSetting(barId, 'RESERVATIONS_MAX_SEATS_PER_SLOT')
  const duration = await getSetting(barId, 'RESERVATIONS_SLOT_DURATION_MINUTES')

  return {
    maxSeatsPerSlot: maxSeats ? parseInt(maxSeats) : (siteConfig.reservations?.maxSeatsPerSlot || 40),
    slotDurationMinutes: duration ? parseInt(duration) : (siteConfig.reservations?.slotDurationMinutes || 120),
  }
}
...
// Simple mutex map for atomicity during local/concurrent reservation requests
const reservationLocks = new Map()

async function acquireReservationLock(key) {
  while (reservationLocks.get(key)) {
    await new Promise(resolve => setTimeout(resolve, 50))
  }
  reservationLocks.set(key, true)
}

function releaseReservationLock(key) {
...
export async function createReservation(data) {
  ...
  const lockKey = `${barId}:${date}:${time}`
  await acquireReservationLock(lockKey)

  try {
    const settings = await getSettings(barId)

    const reservations = await db.collection('reservations').find({ barId, date })
    ...
    releaseReservationLock(lockKey)
  }
}
