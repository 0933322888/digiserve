...
/**
 * Return posts already created for active campaigns.
 * Campaign creation persists concrete posts, so this operation is idempotent.
 */
export async function generatePostsFromCampaigns(barId) {
  const campaigns = await getCampaigns(barId, { status: 'active' })
  const campaignPosts = await Promise.all(
    campaigns.map(campaign => getCampaignPosts(campaign.campaignId, barId))
  )
  return campaignPosts.flat()
}
