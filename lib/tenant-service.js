...
export async function getTenantConfig(barId) {
    ...
    if (tenant) {
        // Ensure legacy color fields are populated from new structure if missing
        const colors = tenant.theme?.colors
        const primaryColor = colors?.light?.primary?.bg || tenant.theme?.primaryColor || '#8B0000'
        const secondaryColor = colors?.light?.secondary?.bg || tenant.theme?.secondaryColor || '#1A1A1A'

        // Inject back into tenant object for consumers expecting legacy fields
        if (tenant.theme) {
            tenant.theme.primaryColor = primaryColor
            tenant.theme.secondaryColor = secondaryColor
        }
